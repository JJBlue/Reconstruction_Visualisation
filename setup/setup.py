...
for pkg in packages:
    if pkg in sys.modules:
        print(f"{pkg!r} already in sys.modules")
        ...
        print(f"Module '{pkg!r}' already installed")
    else:
        print(f"Can't find the {pkg!r} module")
...
if not not_installed_packages:
    ...
    if confirm_dialog("Module installieren"):
        for pkg in not_installed_packages:
            print(f"Installing Module: {pkg!r}")
            subprocess.check_call(["pip", "install", pkg])
            print(f"Module installed: {pkg!r}")